...
namespace WebApplication1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            //remplacer par l'option au dessous(meme dans le post) si vous voulez eliminer l'admin du combobox
        ...
        public async Task<ActionResult> Register(RegisterViewModel model, HttpPostedFileBase upload)
        {
            if (ModelState.IsValid)
            {
                ...
                upload.SaveAs(path);
                ViewBag.UserType = new SelectList(db.Roles,"Name","Name");
                var user = new ApplicationUser { UserName = model.UserName, Email = model.Email,UserType=model.UserType, Birthdate=model.Birthdate,Image=model.Image=upload.FileName };
        ...
        public ActionResult EditProfile(EditProfileViewModel profile, HttpPostedFileBase upload)
        {
            ...
            else
            {
                ...
                CurrentUser.Image = profile.Image=upload.FileName;
                db.Entry(CurrentUser).State = EntityState.Modified;
                db.SaveChanges();
                ViewBag.Message = "Informations Updated Successfully";
                return RedirectToAction("Index", "Home");
